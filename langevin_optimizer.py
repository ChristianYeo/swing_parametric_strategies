...
class pSGLD(optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-3, sigma=1e-6, beta=0.99, Lambda=1e-15, weight_decay=0, centered=False):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, sigma=sigma, beta=beta, Lambda=Lambda, centered=centered, weight_decay=weight_decay)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                noise_std = sigma*sigma*group['lr']/G 
